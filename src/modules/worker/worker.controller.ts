...
export const createProfile = async (req: Request, res: Response) => {
  res.status(201).send({ success: true, data: await svc.createProfile(req.currentUser!.id) });
};
...
export const updateProfile = async (req: Request, res: Response) => {
  res.send({ success: true, data: await svc.updateProfile(req.currentUser!.id, req.body) });
};
...
export const updateEducation = async (req: Request, res: Response) => {
  const result = await svc.updateEducation(req.currentUser!.id, req.params.id as string, req.body);
...
export const updateExperience = async (req: Request, res: Response) => {
  const result = await svc.updateExperience(req.currentUser!.id, req.params.id as string, req.body);
  res.send({ success: true, data: result });
};
...
export const searchWorkers = async (req: Request, res: Response) => {
  ...
  res.send({ success: true, data: await svc.searchWorkers(req.currentUser!, { skillId, city }) });
};
