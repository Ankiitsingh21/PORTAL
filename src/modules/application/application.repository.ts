...
export class ApplicationRepository {
  findByJobAndWorker(jobId: string, workerId: string) {
    return prisma.application.findUnique({ where: { jobId_workerId: { jobId, workerId } } });
  }

  create(data: { jobId: string; workerId: string; recruiterId: string; coverNote?: string }) {
    return prisma.application.create({ data });
  }

  addHistory(data: { applicationId: string; fromStatus?: string; toStatus: string; changedById: string; notes?: string }) {
  ...
  listByJob(jobId: string) {
    return prisma.application.findMany({ where: { jobId }, orderBy: { appliedAt: "desc" } });
  }

  findById(id: string) {
    return prisma.application.findUnique({
      where: { id },
      include: { history: true, worker: { select: { id: true, email: true, phone: true } } },
    });
  }
