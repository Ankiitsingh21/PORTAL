...
export const categoryGuard = async (req: Request, res: Response, next: NextFunction) => {
  if (req.currentUser!.role === "super_admin") return next();

  const industryId = Number(req.body.industryId);
  const assigned = await isRecruiterAssignedToIndustry(req.currentUser!.id, industryId);
